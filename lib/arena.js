...
//		logger.debug('moving '+socket._playerId, direction)
		...
		var collision = this._pixelCollides(nCoords);

		// if there is a collision with a player, not a bonus
		if (collision && !regexp.test(/b_/, collision))
			p.kill();
		// else there is no collision or there is collision with a bonus
			...
			if (regexp.test(/b_/, collision)){
				// yes, so apply the bonus to the player
				this._bonuses.apply(collision, p);
...
//		socket._salt = Math.ceil(Math.random()*100000);
	...
	removeBonus: function(io, socket, bid) {
		logger._debug('removeBonus', bid)
		this._bonuses.del(bid);
		this._socketsSendRemoveBonus(io, bid);
	},
	_socketSendWelcome: function(socket) {
		logger.debug('_socketSendWelcome');
		// sends the memorized grid and players
		socket.emit('welcome', {'arena': this._options, 'players': this._players, 'bonuses': this._bonuses.getBonuses()});
	},
...
//			var k = Math.floor((Math.random()*_bonuses.length));
			var k = number.random(0, _bonuses.length-1);
			var bid = 'b_'+i;
			var bCoords = this._pixelRandom();
//			this._bonuses[bid] = new (require(__dirname+'/bonuses.js'))({'id': bid, 'type'; 'coords': bCoords});
			this._bonuses.add(bid, {'type': _bonuses[k], 'coords': bCoords});
			this._pixelMemorize(bid, bCoords);
		}
		return this;
	},

	_trimPlayerSize: function(p) {
		this._pixelForget(p.getOldestCoords());
		p.shiftCoords();
...
//			'y': Math.floor((Math.random()*(this._options.size.y/cellSize)))
	...
	_pixelCollides: function(coords) {
		return !this._grid || !this._grid[coords.y] || !this._grid[coords.y][coords.x] ? false : this._grid[coords.y][coords.x];
	},
