...
//		logger.debug('moving '+socket._playerId, direction)
	...
	_socketsSendState: function(io, state) {
		logger.debug('_socketsSendState', state);
		io.of('/arena').emit('state', {'state': state});
	},
	...
	_socketsSendGameOver: function(io) {
		...
		io.of('/arena').emit('gameOver', {'msg': 'thy game is done'});
	},
	...
	_checkStartGame: function(io) {
		var oS = this._options.state;
		this._options.state = !this._options.state && object.count(this._players) == this._options.minPlayers ? Math.floor(Date.now() / 1000) : false;
		if (!this._options.state) return;
		logger.debug('_checkStartGame', this._options.state);
		if (!oS && this._options.state) {
			logger.debug('setting gameTime to ', this._options.gameTime*1000);
			this._socketsSendState(io, this._options.state);
			setTimeout(this._socketsSendGameOver.bind(this, io), this._options.gameTime*1000);
		}
	},
