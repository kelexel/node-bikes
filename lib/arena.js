...
module.exports = prime({
	...
	makePlayerId: function(socket, force) {
		if (this._idMap[socket.handshake.cookie['express.sid']] && !force) return this._idMap[socket.handshake.cookie['express.sid']];
		var id = crypto.createHash('md5').update(socket.handshake.sessionID+socket.id+socket._salt).digest("hex");
		this._idMap[socket.handshake.cookie['express.sid']] = id;
		return id;
	},
